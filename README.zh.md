...
[![Release](https://img.shields.io/github/v/release/Adobiz/obsidian-mistake-notebook?color=blue&label=%E6%9C%80%E6%96%B0%E7%89%88)](https://github.com/Adobiz/obsidian-mistake-notebook/releases/)
...
- 📊 **错题仪表盘** — 左侧边栏一键查看：学科分布、数量、复习状态与近半年录入活跃热力格子（GitHub 贡献图风格）。
- 📂 **数据不锁定** — 一道错题就是一个普通 Markdown 笔记 + frontmatter，随时可手改、可被其他插件与脚本处理，迁移零成本。

<p align="center">
  <img src="docs/screenshots/mask.png" width="46%" alt="题目强调框与答案遮罩" />
  <img src="docs/screenshots/dashboard.png" width="46%" alt="错题仪表盘" />
</p>
...
分层约束：领域层 `src/domain` 纯 TypeScript、零依赖（不 import "obsidian"），可在 Node 直接单测；Obsidian API 只出现在适配层与 UI 层。业务规则（拆分阈值、命名、去重）一律进 `src/domain` 并配 `*.test.ts`。
