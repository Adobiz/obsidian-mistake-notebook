...
import { parseMistakeFrontmatter } from "./frontmatter";
...
describe("parseMistakeFrontmatter", () => {
  ...
  it("已移除的 maskStyle（frosted）回退 auto；mosaic/black 合法", () => {
    expect(parseMistakeFrontmatter({ maskStyle: "frosted" }).maskStyle).toBe("auto");
    expect(parseMistakeFrontmatter({ maskStyle: "mosaic" }).maskStyle).toBe("mosaic");
    expect(parseMistakeFrontmatter({ maskStyle: "black" }).maskStyle).toBe("black");
  });
